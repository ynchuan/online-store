  ...
  server.tool(
    ...
    async ({
      ...
      try {
        ...
        // 格式化返回数据
        let result = `美团美食排行榜 (城市ID: ${cityId}, 分类ID: ${cateId})\n`
        if (data.data && Array.isArray(data.data)) {
          result += data.data
            .map(
              (item: any, index: number) =>
