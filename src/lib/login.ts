...
import { CLIENT_ID, CLIENT_SECRET, LOGIN_URL, WX_CLIENT_ID, WX_CLIENT_SECRET, WX_LOGIN_URL, WX_LOGIN_GET_TOKEN, WX_LOGIN_GET_INFO, PDD_BASE_URL, LOGIN_PORT, RC_PATH, LOGIN_REDIRECT_URI } from './const'
...
class BaseLogin {
  ...
  getToken() { return getRc()[this.tokenKey] }
  setToken(data: any) { return setRc({ [this.tokenKey]: data }) }
  clearToken() { this.token = ''; return setRc({ [this.tokenKey]: '' }) }
...
export const githubLogin = new BaseLogin(
  ...
  async (code: string) => {
    const params = { client_id: 'Ov23liUM3HbSFE9QWrer', client_secret: '946dba1a1551289acaa3a9f2b5348dcf3623189e', code }
